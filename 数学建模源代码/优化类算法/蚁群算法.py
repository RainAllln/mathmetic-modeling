# https://blog.csdn.net/dgvv4/article/details/129824758
# 蚁群算法是一种仿照蚂蚁寻食路径的启发式算法，当蚂蚁在搜索食物时，会在蚁巢和食物源的爬行路径上留下一种化学物质，这种化学物质会引导更多的蚂蚁进行更小路径的食物搜索。蚁群算法常常被用来解决最优化问题。
# 利用TSP问题来解释蚁群算法
# 蚂蚁的数量为m,探索n个城市,设城市i到j的距离为d_ij，在t时刻城市i到j之间的信息素浓度为τ_ij(t),一开始时所有路径的信息素相同
# 蚂蚁移动的方向概率是根据信息素以及城市距离组成的概率公式来定义的，当运动的下一个点s属于未探索集合，P^k_ij(t)={[τ_ij(t)]^α*[η_ij(t)]^β/∑s∈allow_k[τ_is(t)]^α*[η_is(t)]^β，如果s已探索，那么概率就是0
# 其中,n_ij为启发信息函数，在TSP问题中是城市间距离的倒数（n_ij=1/d_ij)，α是信息素影响因子，β是启发函数影响因子，根据调整αβ的比例，调整不同因素的影响程度
# 信息素会随时间消散，挥发系数为ρ，当蚂蚁访问完全部的城市（一个循环完成时）再更新路径信息素
# τ_ij(t+1)=(1-ρ)τ_ij(t)+△τ_ij,其中,(1-ρ)τ_ij(t)是留在路径上的信息素挥发，△τ_ij是全部蚂蚁经过这条路径的释放的信息素总和
# 每一只蚂蚁释放的信息素计算：总量为常量Q，L_k为第k只蚂蚁经过的路径总长度△τ_ij^k=Q/L_k,如果蚂蚁没经过(i,j),那么就留下0
# 算法流程：
# Step1:设置算法运行环境，设定初始参数信息。信息素初始浓度τ_ij(0)=C;最大迭代次数 N_max;m只蚂蚁随机分配起始点;信息素浓度增量初始时刻△τ_ij=0。
# Step2:迭代次数N=N+1,(0≤N≤N_max);
# Step3:根据蚂蚁将要访问的城市集合allow_k(k=1,2,…,n-1)进行访问;
# Step4:蚂蚁个数k=k+1,(k=1,2,…,m);
# Step5:蚂蚁k根据公式选择待转移城市;
# Step6:更新待访问城市集合allow_k，将蚂蚁k探索过的城市放入禁忌表;
# Step7:全部的蚂蚁是否都游遍过所有城市节点，即访问过全部城市节点的蚂蚁总数是否小于蚂蚁总个数m，若是，则跳转到Step4;否则按照顺序进入到步骤8;
# Step8:根据公式进行信息素浓度的更新;
# Step9:当前迭代次数是否小于最大迭代次数，即0≤N≤N_max，若是，则跳转到step2;否则按照顺序进入到 Step10;
# Step10:根据运行结果输出最优路径。
